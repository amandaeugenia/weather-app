...
const Forecast = ({ latitude, longitude }) => {
  ...
  useEffect(() => {
    const fetchHourlyForecast = async () => {
      try {
        ...
        console.error('Error fingind dataa:', error.response ? error.response.data : error.message);
      }
    };
  ...
  return (
    <div className="container w-full p-4 bg-white shadow-lg rounded-lg h-[500px] min-h-[350px]">
      <h2 className="text-xl font-bold mb-8">Next Hours</h2>
      <div className="space-y-4">
        {hourlyData.slice(0, 5).map((forecast, index) => (
          <div key={index} className="flex items-center bg-gray-300 p-3 rounded-lg">
            <p className="text-lg w-20 text-center">
